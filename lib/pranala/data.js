...
Data.prototype.getUrlDocByUrl = function(url, successCb, errorCb) {
    this.db.view('content/url_code?key="' + encodeURIComponent(url) + '"', {
        success: successCb,
        error: errorCb
    });
};
Data.prototype.saveUrlDoc = function(code, url, sequence, successCb, errorCb) {
    this.db.saveDoc({
        _id: code, type: 'url', url: url, seq: sequence
    }, {
        success: successCb,
        error: errorCb
    });
