...
url.isBlacklisted = function (url) {
    var blacklist = ['chinababyhome.com', 'chinamama518', 'freeba123.com'],
        domain = url
            .replace(/^https?:\/\/(www\.)?/, '')
            .replace(/\/.*$/, '')
            .replace(/:.*$/, '');
...
url.isShortened = function (url) {
    var shorteners = [
        'bit.ly', 'de.tk', 'j.mp', 'ow.ly', 'ht.ly', 'prn.la',
        'tinyurl.com', 'tr.im', 'is.gd', 'aw.sm', 'pendek.in',
        'goo.gl', 'siteo.us', 'ketkp.in', 'mangk.us', 'bu.tt'
        ],
        domain = url
            .replace(/^https?:\/\//, '')
            .replace(/\/.*$/, '')
...
url.validateShort = function (code) {
    var error = null;
...
url.validateCustom = function (code) {
    var error = null;
