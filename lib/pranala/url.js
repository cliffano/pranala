...
url.isValid = function(url) {
    ...
    if (url && url.match(/^https?:\/\/[0-9a-zA-Z]([-.\w]*[0-9a-zA-Z])*(:[0-9]+)*(\/([a-zA-Z0-9=:\-\.\?\,\'\/\\\+&amp;%\$#_]*))?$/)) {
	    ...
	    if (parsedUrl.host && !parsedUrl.host.match(/.*\..*/)) {
		    isValid = false;
	    }
    }
    return isValid;
...
url.isBlacklisted = function(url) {
    var blacklist = new Array('bit.ly', 'de.tk', 'j.mp', 'ow.ly', 'ht.ly', 'prn.la', 'tinyurl.com', 'tr.im', 'pendek.in', 'siteo.us', 'ketkp.in', 'mangk.us');
